...
const generateTrend = (series) =>
  series.map((v, i) =>
    i < 2 ? (v || 0) : Math.round(((series[i] || 0) + (series[i - 1] || 0) + (series[i - 2] || 0)) / 3)
  )
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-slate-800 px-3.5 py-2.5 rounded-xl text-xs text-white shadow-2xl min-w-[130px]">
      ...
      {payload.map((p, i) => (
        <p key={i} style={{ color: p.color }} className="flex items-center gap-1.5 mt-0.5">
...
const AvatarImg = ({ src, alt = '' }) => {
  ...
  const initials = alt ? alt.split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase() : '??'
  if (errored || !src) {
    return (
      ...
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        >
          <div className="flex items-center gap-2.5">
            <span className="w-1 h-5 rounded-sm" style={{ background: color }} />
              ...
              <thead>
                <tr style={{ background: color + '18' }}>
                  {['Photo', 'Code', 'Name', 'Dept'].map(h => (
                    <th key={h} className="px-4 py-2.5 text-[11px] uppercase tracking-wider font-bold sticky top-0 z-[5] border-b border-slate-200 text-left" style={{ background: color + '18' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {data.map((emp, i) => (
                  <tr key={i} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-2.5 align-middle"><AvatarImg src={emp.photo || emp.img} alt={emp.name} /></td>
                    <td className="px-4 py-2.5 align-middle font-mono text-xs font-bold text-slate-500">{emp.code}</td>
                    <td className="px-4 py-2.5 align-middle font-semibold text-slate-800">{emp.name}</td>
                    <td className="px-4 py-2.5 align-middle">
                      <span className="bg-slate-100 border border-slate-200 px-2 py-0.5 rounded-md text-[11px] text-slate-500">{emp.dept}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="px-5 py-3.5 border-t border-slate-100 text-right flex-shrink-0">
          <button className="bg-slate-100 border-none px-5 py-2 rounded-lg text-[13px] font-semibold cursor-pointer text-slate-700 hover:bg-slate-200 transition-colors" onClick={onClose}>Close</button>
        </div>
      </div>
    </div>
  )
...
const ChartCard = ({ title, accentColor, children, controls }) => (
  <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 h-[460px] flex flex-col gap-0">
    <div className="flex items-center gap-2.5 mb-3">
      <span className="w-1 h-[18px] rounded-sm" style={{ background: accentColor }} />
...
export default function Staff_att() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      rows.forEach(r => r.departments && Object.keys(r.departments).forEach(d => deptSet.add(d)))
  ...
  const openModal = async (type, date) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/${endpoint}/?date=${date}&dept=${dept}`)
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
      const json = await res.json()
      setModal(m => ({ ...m, loading: false, data: Array.isArray(json) ? json : (json.data || []) }))
    } catch {
      setModal(m => ({ ...m, loading: false, data: [] }))
    }
  }

  const closeModal = () => setModal({ type: null, date: null, loading: false, data: null })
  ...
  const avgPct = (arr, key) => arr.length ? (arr.reduce((s, r) => s + (r[key] || 0), 0) / arr.length).toFixed(1) : '0.0'
  ...
  const getDept = (dept, field) => data.map(r => r.departments?.[dept]?.[field] || 0)
  ...
  const exportCSV = () => {
    const headers = ['Date', 'Total', 'Present', 'Present%', 'Absent', 'Absent%', 'Leave', 'Leave%']
    const rows = data.map(r => [
      formatDateToDisplay(r.date), r.total, r.present, r.present_pct,
      r.absent, r.absent_pct, r.leave || 0, r.leave_pct || 0
    ])
    rows.push(['AVERAGE (Excl Sun)',
      avg(nonSundayData, 'total'), avg(nonSundayData, 'present'), avgPct(nonSundayData, 'present_pct'),
      avg(nonSundayData, 'absent'), avgPct(nonSundayData, 'absent_pct'), '', ''
    ])
    ...
    <div className="font-sans min-h-screen bg-[#eef2f7] p-5 flex flex-col gap-5 text-slate-800">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 px-5 py-4 flex flex-wrap items-center gap-4 shadow-sm">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-blue-50 rounded-xl flex items-center justify-center text-blue-600 text-lg">📊</div>
          <div>
            <div className="text-lg font-bold text-slate-800">Staff Attendance</div>
            <div className="text-xs text-slate-500">Analysis for <b className="text-blue-600">{unit}</b></div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5 ml-auto">
          ...
          <div className="flex bg-slate-100 rounded-lg p-0.5 gap-0.5">
            <button className="bg-transparent border-none px-3 py-1 text-xs font-semibold text-slate-500 rounded-md cursor-pointer hover:bg-white hover:text-blue-600 hover:shadow-sm transition-all" onClick={() => setDateRange(1)}>1M</button>
            <button className="bg-transparent border-none px-3 py-1 text-xs font-semibold text-slate-500 rounded-md cursor-pointer hover:bg-white hover:text-blue-600 hover:shadow-sm transition-all" onClick={() => setDateRange(6)}>6M</button>
          </div>

          <button className="bg-transparent border-none cursor-pointer p-1.5 rounded-lg text-slate-400 hover:bg-red-50 hover:text-red-500 transition-all" onClick={clearFilters} title="Clear Filters">✕</button>
      ...
      {nonSundayData.length > 0 && (
        <div className="grid grid-cols-[repeat(auto-fit,minmax(160px,1fr))] gap-3.5">
          <div className="bg-white rounded-xl border border-slate-200 px-4 py-4 flex flex-col gap-1.5 shadow-sm">
            <div className="text-[11px] uppercase tracking-widest text-slate-400 font-semibold">Avg Onroll</div>
            <div className="text-3xl font-extrabold leading-none text-blue-600">{avg(nonSundayData, 'total')}</div>
            <div className="text-xs text-slate-400">Excl. Sundays</div>
          </div>
          <div className="bg-white rounded-xl border border-slate-200 px-4 py-4 flex flex-col gap-1.5 shadow-sm">
            <div className="text-[11px] uppercase tracking-widest text-slate-400 font-semibold">Avg Present</div>
            <div className="text-3xl font-extrabold leading-none text-emerald-600">{avg(nonSundayData, 'present')}</div>
            <div className="text-xs text-slate-400">{avgPct(nonSundayData, 'present_pct')}% attendance</div>
          </div>
          <div className="bg-white rounded-xl border border-slate-200 px-4 py-4 flex flex-col gap-1.5 shadow-sm">
            <div className="text-[11px] uppercase tracking-widest text-slate-400 font-semibold">Avg Absent</div>
            <div className="text-3xl font-extrabold leading-none text-rose-600">{avg(nonSundayData, 'absent')}</div>
            <div className="text-xs text-slate-400">{avgPct(nonSundayData, 'absent_pct')}% absenteeism</div>
          </div>
          <div className="bg-white rounded-xl border border-slate-200 px-4 py-4 flex flex-col gap-1.5 shadow-sm">
            <div className="text-[11px] uppercase tracking-widest text-slate-400 font-semibold">Days Tracked</div>
            <div className="text-3xl font-extrabold leading-none text-violet-600">{data.length}</div>
            <div className="text-xs text-slate-400">{nonSundayData.length} working days</div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-5 max-[900px]:grid-cols-1">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col h-[500px] overflow-hidden">
          <div className="px-4 py-3.5 bg-slate-50 border-b border-slate-200 rounded-t-2xl flex justify-between items-center flex-shrink-0">
            <span className="text-xs font-bold uppercase tracking-widest text-slate-500">Daily Log</span>
            <button
          ...
          <div className="overflow-y-auto flex-1 [&::-webkit-scrollbar]:w-[5px] [&::-webkit-scrollbar-track]:bg-slate-100 [&::-webkit-scrollbar-thumb]:bg-slate-300 [&::-webkit-scrollbar-thumb]:rounded">
            <table className="w-full border-collapse text-[13px]">
              <thead>
                <tr>
                  <th rowSpan={2} className="sticky top-0 z-10 bg-white px-3 py-2.5 text-[10px] uppercase tracking-widest font-bold text-slate-400 border-b border-slate-100 text-left">Date</th>
                  <th rowSpan={2} className="sticky top-0 z-10 bg-white px-3 py-2.5 text-[10px] uppercase tracking-widest font-bold text-slate-400 border-b border-slate-100 text-center">Total</th>
                  <th colSpan={2} className="sticky top-0 z-10 bg-white px-3 py-2.5 text-[10px] uppercase tracking-widest font-bold text-emerald-600 border-b border-slate-100 text-center">Present</th>
                  <th colSpan={2} className="sticky top-0 z-10 bg-white px-3 py-2.5 text-[10px] uppercase tracking-widest font-bold text-rose-600 border-b border-slate-100 text-center">Absent</th>
                  <th colSpan={2} className="sticky top-0 z-10 bg-white px-3 py-2.5 text-[10px] uppercase tracking-widest font-bold text-amber-600 border-b border-slate-100 text-center">Leave</th>
                </tr>
                <tr>
                  {['Count', '%', 'Count', '%', 'Count', '%'].map((h, i) => (
                    <th key={i} className="sticky top-[37px] z-10 bg-white px-3 py-2.5 text-[10px] uppercase tracking-widest font-bold text-slate-400 border-b border-slate-100 text-center">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {data.map((row, i) => (
                  ...
                  >
                    <td className={`px-3 py-2.5 font-semibold whitespace-nowrap ${isSunday(row.date) ? 'text-purple-700 font-bold' : 'text-slate-700'}`}>
                    ...
                    <td className="px-3 py-2.5 text-center">
                      <span className="inline-block px-2 py-0.5 rounded-md text-[11px] font-bold bg-emerald-100 text-emerald-700">{row.present_pct}%</span>
                    </td>
                    <td
                    ...
                    >{row.absent}</td>
                    <td className="px-3 py-2.5 text-center">
                      <span className="inline-block px-2 py-0.5 rounded-md text-[11px] font-bold bg-rose-100 text-rose-700">{row.absent_pct}%</span>
                    </td>
                    <td className="px-3 py-2.5 text-center font-bold text-amber-600">{row.leave || 0}</td>
                    <td className="px-3 py-2.5 text-center">
                      <span className="inline-block px-2 py-0.5 rounded-md text-[11px] font-bold bg-orange-100 text-orange-700">{row.leave_pct || 0}%</span>
                    </td>
                  </tr>
                ))}
              </tbody>
              {nonSundayData.length > 0 && (
                <tfoot>
                  <tr className="bg-slate-100 border-t-2 border-slate-200">
                    <td className="px-3 py-2.5 text-left font-bold text-xs">AVERAGE</td>
                    <td className="px-3 py-2.5 text-center font-bold text-xs text-blue-600">{avg(nonSundayData, 'total')}</td>
                    <td className="px-3 py-2.5 text-center font-bold text-xs text-emerald-600">{avg(nonSundayData, 'present')}</td>
                    <td className="px-3 py-2.5 text-center font-bold text-xs text-emerald-600">{avgPct(nonSundayData, 'present_pct')}%</td>
                    <td className="px-3 py-2.5 text-center font-bold text-xs text-rose-600">{avg(nonSundayData, 'absent')}</td>
                    <td className="px-3 py-2.5 text-center font-bold text-xs text-rose-600">{avgPct(nonSundayData, 'absent_pct')}%</td>
                    <td className="px-3 py-2.5 text-center font-bold text-xs">{avg(nonSundayData, 'leave')}</td>
                    <td className="px-3 py-2.5 text-center font-bold text-xs">{avgPct(nonSundayData, 'leave_pct')}%</td>
                  </tr>
                </tfoot>
              )}
            </table>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
        ...
        <div className="h-[380px]">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={chartData.filter((_, i) => !isSunday(data[i]?.date))} margin={{ top: 16, right: 8, bottom: 8, left: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
              <XAxis dataKey="date" tick={{ fontSize: 10, fill: '#94a3b8' }} tickLine={false} axisLine={false} interval="preserveStartEnd" />
              <YAxis tick={{ fontSize: 10, fill: '#94a3b8' }} tickLine={false} axisLine={false} />
              <Tooltip content={<CustomTooltip />} />
