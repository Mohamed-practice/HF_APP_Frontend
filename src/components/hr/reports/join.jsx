...
import { 
  ArrowLeft, Building2, UserPlus, FileSpreadsheet, 
  X, Filter, FolderOpen, Keyboard, Calendar 
} from 'lucide-react';
import * as XLSX from 'xlsx';
import { useNavigate } from 'react-router-dom';

const API_BASE_URL = 'http://10.1.21.13:8200/reports/join_data/';
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-slate-100 font-sans">
      ...
      <nav className="bg-white border-b border-slate-200 flex-none px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-800 tracking-tight">Joining Dashboard</h1>
              <p className="text-xs text-slate-500 font-medium">New Employee Onboarding</p>
            </div>
          </div>
          <button onClick={() => setShowFilterModal(true)} className="lg:hidden p-2 text-slate-600">
            <Filter className="w-6 h-6" />
          </button>
          <div className="hidden lg:flex items-center gap-3">
            <button onClick={() => navigate(-1)} className="text-slate-500 hover:text-indigo-600 font-medium text-sm flex items-center gap-1 transition-colors">
      ...
      <div className="hidden lg:block bg-white border-b border-slate-200 shadow-sm z-20 flex-none py-3 px-4 sm:px-6 lg:px-8">
        <div className="flex items-end gap-3 flex-wrap">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[11px] font-semibold text-slate-500 uppercase">Date Range</label>
          ...
          <div className="flex gap-2 pb-1">
            {['1M', '3M'].map(m => (
              <button key={m} onClick={() => setQuickDate(parseInt(m))} className="px-3 py-1.5 text-xs font-medium text-slate-600 bg-white border border-slate-300 rounded-full hover:bg-indigo-50 hover:text-indigo-600">
                {m}
              </button>
            ))}
          </div>
      ...
      <main className="flex-1 flex flex-col min-h-0 w-full max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-4 overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 flex-none">
          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between group">
            <div>
              <p className="text-xs font-semibold text-slate-500 uppercase">Total Joining</p>
              <h3 className="text-2xl font-bold text-slate-800">{loading ? '...' : employees.length}</h3>
            </div>
          ...
          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between group">
            <div className="overflow-hidden">
              <p className="text-xs font-semibold text-slate-500 uppercase">Selected Unit</p>
              <h3 className="text-xl font-bold text-slate-800 truncate">{filters.unit}</h3>
            </div>
        ...
        <div className="flex-1 bg-white border border-slate-300 rounded-xl shadow-sm overflow-hidden flex flex-col">
          <div className="px-6 py-3 border-b border-slate-200 bg-slate-50 flex justify-between items-center flex-none">
            <h2 className="text-sm font-bold text-slate-700 uppercase tracking-wide">Joining List</h2>
            <button 
              onClick={exportExcel}
              ...
              <table className="w-full text-left border-collapse border-spacing-0">
                <thead className="bg-slate-50 sticky top-0 z-10 shadow-sm">
                  <tr className="divide-x divide-slate-300">
                    {['No', 'ID', 'Photo', 'Name', 'Designation', 'Department', 'Joining Date'].map(header => (
                      <th key={header} className="px-4 py-3 text-xs font-bold text-slate-600 uppercase border-b border-slate-300 bg-slate-50">
                        {header}
                      </th>
                    ))}
                  </tr>
                  ...
                  {employees.map((emp, rIdx) => (
                    <tr key={emp.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      {[
                        rIdx + 1,
                        emp.empcode || '-',
                        <div className="flex justify-center" key={`img-${emp.id}`}>
                          {emp.photo ? (
                            <img src={emp.photo} alt="" className="w-8 h-8 rounded-full object-cover border border-slate-200 shadow-sm" />
                          ) : (
                        ...
                        emp.designation || '-',
                        emp.dept || '-',
                        <span className="font-bold text-slate-700" key={`date-${emp.id}`}>
                          {emp.joindt ? new Date(emp.joindt).toLocaleDateString('en-GB') : '-'}
                        </span>
                      ].map((content, cIdx) => (
                        <td 
                          key={cIdx} 
      ...
      <div className={`fixed bottom-6 right-6 bg-slate-900 text-white px-4 py-2 rounded-full text-xs font-medium flex items-center gap-2 shadow-2xl transition-all duration-300 z-50 ${showNavIndicator ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
      ...
      {showFilterModal && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm" onClick={() => setShowFilterModal(false)} />
          <div className="relative bg-white w-full max-w-lg rounded-t-3xl sm:rounded-2xl shadow-2xl overflow-hidden">
            <div className="px-6 py-4 border-b flex justify-between items-center">
              ...
              <button onClick={() => setShowFilterModal(false)} className="p-2 bg-slate-100 rounded-full"><X className="w-5 h-5"/></button>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input type="date" value={filters.start} onChange={(e) => setFilters({...filters, start: e.target.value})} className="w-full p-3 bg-slate-50 border rounded-xl" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 uppercase">To</label>
                  <input type="date" value={filters.end} onChange={(e) => setFilters({...filters, end: e.target.value})} className="w-full p-3 bg-slate-50 border rounded-xl" />
                </div>
              </div>
              <button onClick={() => setShowFilterModal(false)} className="w-full py-4 bg-indigo-600 text-white font-bold rounded-xl">Apply Filters</button>
            </div>
          </div>
        </div>
      )}
